...
use Order\Tracking\Helper\PickupStoreAddressHelper;
...
class Invoice extends \Magento\Sales\Model\Order\Pdf\Invoice
{
    ...
    /**
     * @var PickupStoreAddressHelper
     */
    private $pickupStoreAddressHelper;
    ...
    public function __construct(
        ...
        PickupStoreAddressHelper $pickupStoreAddressHelper,
        ...
        $this->pickupStoreAddressHelper = $pickupStoreAddressHelper;
    ...
    public function getPdf($sellerinvoices = [])
    {
       ...
       // $this->_setFontBold($style, 10);
        $font = \Zend_Pdf_Font::fontWithPath(
            $this->_rootDirectory->getAbsolutePath('lib/internal/PyiDaungSuFont/ZawgyiOne2008.ttf')
        );
       ...
       // $this->_setFontRegular($sellerPdfPage, 10);
        ...
        if ($this->helper->getSellerProfileDisplayFlag()) {
            ...
            if (!$sellerOrder->getIsVirtual()) {
                ...
                if ($sellerOrder->getPayAtStoreId() && $sellerOrder->getShippingMethod() == 'payatstore_payatstore') {
                    $pickupStoreAddress = $this->_formatAddress(
                        $this->pickupStoreAddressHelper->generateAddress($sellerOrder->getPayAtStoreId(), 'pdf')
                    );
    
    
                    foreach ($pickupStoreAddress as $value) {
                        if ($value !== '') {
                            $text = [];
    ...
    protected function insertAddress(&$sellerPdfPage, $store = null)
    {
        ...
        // $address = ''; // M@gent0!202! // php bin/magento c:c && php bin/magento c:f 
        // cd /var/www/html/magento_demo/app/code/Webkul/Marketplace/Model/Order/Pdf
        // vim Invoice.php  
        $address = '';
        $sellerId = $this->helper->getCustomerId();
        // get seller data for store in which order is placed
